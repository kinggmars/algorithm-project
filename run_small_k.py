"""
run_small_k.py  —— 小 k (k ≤ 11) 马尔可夫模型分类管线（稠密矩阵形式）
...
用法：
    ...
    # 扫描 k=3..dense_threshold 找最优 k
    python run_small_k.py --genomes ./proj1/genomes \\
                          --test ./proj1/test/test.fa --map ./proj1/test/seq_id.map --sweep

    # 全范围扫描 k=3..20（自动切换稠密/稀疏，带内存保护）
    python run_small_k.py --genomes ./proj1/genomes \\
                          --test ./proj1/test/test.fa --map ./proj1/test/seq_id.map --full-sweep
...
def main():
    parser = argparse.ArgumentParser(description="小 k 马尔可夫模型分类（稠密矩阵 + 置换检验）")
    ...
    parser.add_argument("--sweep", action="store_true",
                        help=f"扫描 k=3..{K_DENSE_THRESHOLD} 寻找最优 k")
    ...
    if args.sweep:
        ...
        results = sweep_k(args.genomes, args.test, args.map,
                          ...
                          n_permutations=args.n_permutations)
        timer.stop("扫描")
        print_sweep_results(results, f"小 k 值扫描结果 (k=3..{args.dense_threshold})")
        timer.summary()
        return
